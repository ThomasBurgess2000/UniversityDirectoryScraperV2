...
driver.get("https://www.biola.edu/directory/search?_type=person")

#GO TO LOGIN PAGE
loginLink=driver.find_element_by_xpath('//*[@id="biolaHeader"]/div/div/a')
loginLink.click()
time.sleep(3)
...
for t in range(1,371):
    tStr=str(t)
    driver.get("https://www.biola.edu/directory/search?_type=person&page="+tStr)
    ...
    # to the dictionary 'BiolaDirectory' where the key is the name and the value is a list containing the information
    ...
    while (i<len(pageListOfImageLinks)):
        ...
        if (name=="Tom Keene"):
            image="https://apps.biola.edu/idphotos/d36ec5f30a0cd006d11f2c953ac844fb_medium.jpg"
            i-=1
    ...
    while (i<len(pageListOfEmails)):
        ...
        if (pageListOfNames[i].text=='Tom Keene'):
            BiolaDirectory['Tom Keene']=['',"https://apps.biola.edu/idphotos/d36ec5f30a0cd006d11f2c953ac844fb_medium.jpg"]
            p-=1
        else:
            BiolaDirectory[pageListOfNames[i].text]=[fixedPageListOfEmails[i],fixedPageListOfImages[p]]
        ...
        x=BiolaDirectory[pageListOfNames[i].text]
...
with open('bioladirectory.txt','w+') as output:
    while (i<len(finalListOfEmails)):
        output.write("%s\n" % str(i+1))
        output.write("%s\n" % str(finalListOfNames[i]))
        output.write("%s\n" % str(finalListOfEmails[i]))
        if (str(finalListOfNames[i])=='Tom Keene'):
            output.write("%s\n" % "https://apps.biola.edu/idphotos/d36ec5f30a0cd006d11f2c953ac844fb_medium.jpg")
            p-=1
        else:
